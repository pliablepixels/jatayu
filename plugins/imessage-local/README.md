# iMessage (Jatayu fork)

> **Attribution.** This is a vendored fork of Anthropic's official
> iMessage plugin, originally distributed in
> [`claude-plugins-official`](https://claude.ai/code) at
> `external_plugins/imessage`. Copyright 2026 Anthropic, PBC — licensed
> under Apache-2.0 (see [`LICENSE`](./LICENSE), preserved unchanged).
>
> **Modifications made for Jatayu:** the upstream plugin drops the
> Owner's own outgoing messages in chats it serves, so the bot only sees
> one side of the conversation. This fork delivers the Owner's outbound
> messages back to the session so the bot sees both sides (required for
> the context/memory pipeline in [`docs/memory.md`](../../docs/memory.md)).
> Track upstream at the source above and re-apply modifications when
> syncing.
