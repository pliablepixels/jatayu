...
// Comma-separated list of names the bot answers to (in addition to "claude").
// Set via IMESSAGE_BOT_NAMES so the actual name isn't hardcoded in checked-in source.
const BOT_NAMES = [
  'claude',
  ...(process.env.IMESSAGE_BOT_NAMES ?? '')
    .toLowerCase()
    .split(',')
    .map(s => s.trim())
    .filter(Boolean),
]
const BOT_NAME_RE = new RegExp(
  `(?:^|[\\s,])@?(?:${BOT_NAMES.map(n => n.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')).join('|')})\\b`,
  'i',
)
const SIGNATURE = '\nSent by Claude'
// Owner's configured signature from PERSONAL.yaml (bot.signature). Model
// appends it in reply text; trailing whitespace drift in chat.db can make
// the echoKey miss otherwise, so we strip it aggressively in echoKey.
const OWNER_SIGNATURE = (process.env.IMESSAGE_OWNER_SIGNATURE ?? '').trim()
const OWNER_SIG_RE = OWNER_SIGNATURE
  ? new RegExp(`\\s*${OWNER_SIGNATURE.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*$`)
  : null
const CHAT_DB =
...
function gate(input: GateInput): GateResult {
  ...
  if (!input.isGroup) {
    ...
    // Cap pending slots to limit DoS surface. If full, evict the oldest
    // entry instead of silently dropping new unknown senders — keeps the
    // queue responsive to fresh attempts while still bounded.
    const MAX_PENDING = 3
    const pendingEntries = Object.entries(access.pending)
    if (pendingEntries.length >= MAX_PENDING) {
      const [oldestCode] = pendingEntries.reduce((a, b) =>
        a[1].createdAt <= b[1].createdAt ? a : b,
      )
      delete access.pending[oldestCode]
    }
    ...
    access.pending[code] = {
      ...
      createdAt: now,
      // 10 min TTL — legitimate pairings complete in seconds; shorter TTL
      // shrinks the window for a DoS holding pending slots hot.
...
function echoKey(raw: string): string {
  let s = raw.replace(/\s*Sent by Claude\s*$/, '')
  if (OWNER_SIG_RE) s = s.replace(OWNER_SIG_RE, '')
  return s
...
function sendText(chatGuid: string, text: string): string | null {
  // Track before invoking osascript: Messages.app occasionally returns a
  // non-zero status on successful delivery, which would skip tracking and
  // let the bot's own echo come back through the poll and trigger a reply.
  // Stale entries age out of `echo` on their own (ECHO_WINDOW_MS).
...
// Deduplicate by message GUID — the same message can appear in multiple chat
// records (e.g. phone-number chat vs Apple ID chat for the same DM thread).
...
function poll(): void {
  ...
  for (const r of rows) {
    ...
    if (deliveredContent.has(contentKey)) continue
...
function handleInbound(r: Row): void {
  ...
  // Allow outgoing messages if: the chat is explicitly configured in groups (opt-in),
  // OR it's a self-chat DM (DM participant is the bot's own address — in a self-chat
  // all messages are is_from_me=1, so we can't rely on an is_from_me=0 copy),
  // OR @claude/@agentname is explicitly tagged. All other sent rows are dropped.
  if (r.is_from_me) {
    process.stderr.write(`imessage channel: [debug] outgoing msg — chat=${r.chat_guid} text=${JSON.stringify(text)} rowid=${r.rowid}\n`)
    const _access = loadAccess()
    const inConfiguredGroup = !!_access.groups[r.chat_guid]
    // For DM chats (any;-;<address>), use the DM participant's address as user
    // so the client accepts the notification (it validates user matches chat_id for DMs).
    // For group chats, fall back to ownerAddr.
    const dmAddrMatch = r.chat_guid.match(/^any;-;(.+)$/)
    const isSelfChatOutgoing = !!dmAddrMatch && SELF.has(dmAddrMatch[1].toLowerCase())
    if (!inConfiguredGroup && !isSelfChatOutgoing && !(isGroup && BOT_NAME_RE.test(text))) return
    // Drop echoes of messages the bot itself just sent (sendText tracks them).
    if (consumeEcho(r.chat_guid, text || '\x00att')) return
    const ownerAddr = [...SELF][0] ?? 'me'
    const notifUser = ownerAddr
    ...
    // Self-chat messages often land twice in chat.db (an is_from_me=1 copy
    // and an is_from_me=0 copy with different GUIDs, or two is_from_me=1
    // copies when synced across Apple-ID devices). Track the text so the
    // sibling row is caught by consumeEcho on the other delivery path.
    if (isSelfChatOutgoing) trackEcho(r.chat_guid, text || '\x00att')
    return
  }
  ...
  // Group-chat gate: only deliver messages that are clearly for the bot.
  // Saves tokens (model doesn't evaluate every group message) and shrinks
  // the prompt-injection surface. Bot-name mention or @claude is required.
  // DMs pass through — the sender is already allowlisted.
  if (isGroup && !BOT_NAME_RE.test(text)) return
  ...
  // See note on self-chat twin rows in the outgoing branch above. Track
  // here too so the is_from_me=1 sibling is dropped by consumeEcho.
  if (isSelfChat) trackEcho(r.chat_guid, text || '\x00att')
}
