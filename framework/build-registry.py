#!/usr/bin/env python3
"""
build-registry.py — Builds framework/autogen-registry.json from plugins and channels.

Also writes:
  framework/autogen-channels       — comma-separated plugin strings for start.sh --channels flag
  framework/autogen-channel-rules.md — concatenated channel.md files for Claude to read

All three output files are auto-generated and gitignored. Do not edit them directly
— they will be overwritten on the next bot start.

Called by framework/loader.sh on every bot start. Skips the rebuild when
all source files are older than the existing autogen-registry.json.
...
REGISTRY_FILE = FRAMEWORK_DIR / "autogen-registry.json"
CHANNELS_FILE = FRAMEWORK_DIR / "autogen-channels"
CHANNEL_RULES_FILE = FRAMEWORK_DIR / "autogen-channel-rules.md"
...
def main() -> int:
    ...
    autogen_comment = "DO NOT EDIT — generated by framework/build-registry.py. Will be overwritten automatically on next bot start."
    registry["_autogenerated"] = autogen_comment
    ...
    channel_rules_header = f"<!-- DO NOT EDIT — generated by framework/build-registry.py. Will be overwritten automatically on next bot start. -->\n\n"
    with open(CHANNEL_RULES_FILE, "w") as f:
        f.write(channel_rules_header)
        f.write("\n\n---\n\n".join(channel_rules_parts) if channel_rules_parts else "")
    print(f"  autogen-channel-rules.md: {len(channel_rules_parts)} rule file(s) merged")
