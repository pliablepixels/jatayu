...
Each morning, after the briefing is sent, Jatayu creates one-shot tasks for external appointments so the Owner receives a timely "leave now" alert with drive-time context for each event requiring travel.
